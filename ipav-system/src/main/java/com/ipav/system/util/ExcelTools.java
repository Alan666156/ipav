...
public class ExcelTools {
	 ...
	 * @param cell
	 */
	public void setCellValue(int type, Object value, Cell cell) {
		if (type == ContentUtil.EXCEL_TYPE_INT)
			cell.setCellValue(this.getIntValue(value));
		else if (type == ContentUtil.EXCEL_TYPE_FLOAT)
			cell.setCellValue(this.getFloatValue(value));
		else
			cell.setCellValue(this.getStringValue(value));
	}
	 ...
	 * @return
	 */
	private int getCellValueType(Object obj) {
	 ...
	 * @param type
	 */
	private void setCellType(Cell cell, int type) {
		...
		if (content != null&&content.size()>0) {
			...
			for (int i = 0; i < rowLength; i++) {
				...
				for (int j = 0; j < cellLength; j++) {
					...
					cellValueType = this.getCellValueType(cellValue);
...
//		}
		while (rows.hasNext()) {
			...
			for(int i=0;i<length;i++){
				cell=row.getCell(i);
				if(i==0&&(cell==null||"".equals(this.getCellValue(cell))))
					return;
				if(cell==null)
					cellValues.add("");
				else
