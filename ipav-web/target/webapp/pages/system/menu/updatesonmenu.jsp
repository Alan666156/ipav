...
<div class="centmenudiv">
	<ol class="centmenu" id="centmenu">
    	<li><a href="#1" onClick="set_menu3(0)" class="current">修改子应用菜单</a></li>
    </ol>
</div>
...
<table class="qytable mt30"  border="0" cellpadding="0" cellspacing="0">
    ...
    <select class="tjsele {required:true}" name="parentid" >
    	<c:if test="${!empty menus}">
    		<c:forEach items="${menus}" var="item">
	        	<option value="${item.menuid }" <c:if test="${item.menuid eq menu.parentid}">selected</c:if>>
	        	<c:out value="${item.menuname}"/></option>
	        </c:forEach>
        </c:if>
    </select></td>
  </tr>
  <tr height="60">
    ...
    <td><input type="text" class="sytext2 {required:true,maxlength:20}" name="menuname" value="${menu.menuname }"></td>
  </tr>
   <tr height="60">
